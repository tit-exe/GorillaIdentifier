...
This repository contains only the V1 training pipeline: extraction, training, and gallery construction. It does not include photos, crops, or model weights. Those are either regenerated locally or downloaded from HuggingFace.

> **Android app** is in a separate repository: https://github.com/tit-exe/GorillaIdentifier_AndroidApp
...
![Benchmark on the held-out validation set](docs/figures/benchmark.png)
...
The gorilla face detector and the trained V1 model are hosted on HuggingFace at
[tit0000/GorillaIdentifier](https://huggingface.co/tit0000/GorillaIdentifier). Fetch the detector with:

```bash
python models/download_models.py
```

This downloads `models/yolo_gorilla.pt`, which crop extraction needs. Add `--version all` to also fetch the trained checkpoint, the gallery, and the TFLite files. The MegaDescriptor-T-224 backbone does not need a manual download: it is fetched automatically from the HuggingFace Hub the first time `v1_megadesc_arcface/train.py` runs, and cached locally afterward (see `config.yaml` to change the cache location).
...
![Crops per individual in the V1 dataset](docs/figures/dataset.png)
...
├── models/                            Not tracked by git, download from HuggingFace
│   ├── README.md                      Explains what goes here and where to get it
│   ├── download_models.py             Fetches the weights from HuggingFace
│   └── yolo_gorilla.pt
...
│   └── diagnostics.json
├── export_tflite_gorilla.py           Exports the backbone + detector to TFLite (WSL, for the app)
...
├── ADDING_INDIVIDUALS.md              How the gallery works and how to add individuals safely
...
[gallery.json]                      max cosine similarity over up to 25 exemplars per individual
    |
    v
similarity >= 0.4689 and margin >= 0.08   ->   individual identified, with a score
similarity < 0.4689 or margin < 0.08      ->   "Unknown"
```
